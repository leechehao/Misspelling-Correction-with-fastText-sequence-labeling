...
def prepare_misspelling_label(
    ...
    Args:
        ...
        max_num_typo (int): The maximum number of typos allowed in the sentence.
        word2id (Dict[str, int]): A dictionary mapping words to their IDs.
    ...
    valid_mask = np.array([is_valid_word(word, word2id, min_num_characters, stop_words) for word in words])
    if not is_typo or not any(valid_mask):
        label_data.append(
            (
                words,
                words,
                [word2id[word.lower()] if word.lower() in word2id else 0 for word in words],
                len(words),
            )
        )
        return label_data
...
def main():
    ...
    with open(args.input_file, "r", encoding="utf-8") as finput:
        for sentence in tqdm(finput.readlines()):
